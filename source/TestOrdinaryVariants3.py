...
def print_cohomology_dim_bridgeless_tex(v_range, l_range, even_edges):
    ...
    for l in l_range:
        ...
        for v in v_range:
            ...
            d = VV.get_dimension()
            if VV.is_valid():
                r1 = D1.get_matrix_rank() if D1.get_domain().is_valid() and D1.get_target().is_valid() else 0
                r2 = D2.get_matrix_rank() if D2.get_domain().is_valid() and D2.get_target().is_valid() else 0
                cdim = str(d-r1-r2)
            else:
                cdim = "-"
            print("& ", cdim, end=" ")
