...
def combirank3(op1, op2, op3):
    ...
    if op2.is_valid() and op2.domain.get_dimension() > 0 and op2.target.get_dimension() > 0:
        B = op2.get_matrix()
    if op3.is_valid() and op3.domain.get_dimension() > 0 and op3.target.get_dimension() > 0:
        C = op3.get_matrix()
    if A and B and C:
        return A.augment(B).augment(C).rank()
    elif A and B:
        return A.augment(B).rank()
    elif A and C:
        return A.augment(C).rank()
    elif B and C:
        return B.augment(C).rank()
    elif A:
        return A.rank()
    elif B:
        return B.rank()
...
def cohom_dim2(nv, nl):
    ...
    opn1 = BVCyclic.AddVReconnectEdgesGO.generate_operator(nv-1,nl+1)
    opn2 = BVCyclic.AddVReconnectEdgesGO.generate_operator(nv-2,nl+1)
    ...
    if opr2.is_valid() and opr2.domain.get_dimension()>0 and opr2.target.get_dimension()>0:
        rc2 = combirank(opr2, opn2)
        # if opr2.exists_rank_file():
        #     rc2 = opr2.get_matrix_rank()
        # else:
        #     return "?o"
    r1 = combirank3(opc1, opr1, opn1)
    r2 = combirank3(opc2, opr2, opn2)
