...
if ForestedGraphComplex.use_bridgeless:
    raise RuntimeError("ForestedBridgelessTransformer: Use this module only if use_bridgeless is False!")
...
for h in h_range:
    for l in l_range:
        for v in v_range:
            for m in m_range:
                bm = BridgelessMask(ForestedGraphComplex.PreForestedGVS(v,l,m,h))
                bm.create_filtered_basisfile()
                for even_edges in [True, False]:
                    bm = BridgelessMask(ForestedGraphComplex.ForestedGVS(v,l,m,h,even_edges))
                    bm.create_filtered_basisfile()
...
for h in h_range:
    for l in l_range:
        for v in v_range:
            for m in m_range:
                for even_edges in [True, False]:
                    bo = BridgeLessMaskOM(ForestedGraphComplex.ContractEdgesGO.generate_operator(v,l,m,h,even_edges))
                    bo.create_filtered_matrixfile()
                    bo = BridgeLessMaskOM(ForestedGraphComplex.UnmarkEdgesGO.generate_operator(v,l,m,h,even_edges))
                    bo.create_filtered_matrixfile()                    
